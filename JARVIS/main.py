...
def commands(isEntryAllowed=False):
    ...
    while isEntryAllowed==True:
        ...
        if 'search for Wi-Fi' in user_input:
            ...
            from pywhatkit import sendwhatmsg
            print("speak mobile number to which you want to send")
            speak('speak mobile number to which you want to send')
            ...
            while True:
                ...
                cv2.imshow('my try to detect faces live',frame)
                key = cv2.waitKey(1)
                if key==81 or key==113:
                    del(camera)
                    break
            ...
            speak('going to shut  down your system in 30 seconds')
            shutdown(time=30)
        elif 'play Tic Tac Toe' in user_input:
            from os import system
...
def commands(isEntryAllowed=False):
    ...
    while isEntryAllowed==True:
        ...
        if 'Wi-Fi' in user_input:
            ...
            speak('connecting to nearby network')
            from os import system
            system('netsh wlan connect name=""')
            speak('Connection request was completed successfully')
